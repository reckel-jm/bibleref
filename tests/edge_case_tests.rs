use bibleref::{
    ...
    parse, translate,
    referencing::language::{get_reference_representation_in_language, BookReferenceType},
};
...
fn test_books_with_one_chapter() {
    ...
    for book in one_chapter_books {
        ...
        if let Ok(BibleReferenceRepresentation::Single(BibleReference::BibleVerse(verse_ref))) = parsed_verse_only {
            assert_eq!(
                verse_ref.chapter(), 1,
                "Expected chapter 1 for '{}', got {}",
                verse_only, verse_ref.chapter()
            );
            assert_eq!(
                verse_ref.verse(), 5,
                "Expected verse 5 for '{}', got {}",
                verse_only, verse_ref.verse()
            );
        } else {
...
fn test_chapters_with_varying_verse_counts() {
    ...
    for (book, chapter, verse_count) in test_cases {
        ...
        assert_eq!(
            get_number_of_verses(&book, &chapter).unwrap(), verse_count,
...
fn test_references_at_boundaries() {
    ...
    assert_eq!(
        genesis_book_ref.previous(), None,
        "There should be no book before Genesis"
    );
    
    // Test that there is no book after Revelation
    let revelation_book_ref = BibleReference::BibleBook(BibleBookReference::new(BibleBook::Revelation));
    assert_eq!(
        revelation_book_ref.next(), None,
...
fn test_malformed_references() {
    ...
    let malformed_references = vec![
        ...
        // Non-existent books
        "Hezekiah 1:1", "3 Corinthians 1:1", "Book of Mormon 1:1",
        
        // Non-existent chapters
        "Genesis 51:1", "Revelation 23:1", "Jude 2:1",
