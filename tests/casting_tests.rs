...
fn test_upcast_verse_range_to_chapter() {
    ...
    for book in books {
        ...
        let verse_range_representation = BibleReferenceRepresentation::Range(BibleRange::VerseRange(verse_range));
        let chapter_reference = BibleChapterReference::new(book, chapter_num).unwrap();
        let chapter_representation = BibleReferenceRepresentation::Single(BibleReference::BibleChapter(chapter_reference));
...
fn test_upcast_chapter_range_to_book() {
    ...
    for book in books {
        ...
        let chapter_range_representation = BibleReferenceRepresentation::Range(BibleRange::ChapterRange(chapter_range));
        let book_reference = BibleBookReference::new(book);
        let book_representation = BibleReferenceRepresentation::Single(BibleReference::BibleBook(book_reference));
...
fn test_upcast_partial_verse_range() {
    ...
    for (book, chapter, start_verse, end_verse) in test_cases {
        ...
        let verse_range_representation = BibleReferenceRepresentation::Range(BibleRange::VerseRange(verse_range.clone()));
        let expected_representation = BibleReferenceRepresentation::Range(BibleRange::VerseRange(verse_range));
...
fn test_upcast_partial_chapter_range() {
    ...
    for (book, start_chapter, end_chapter) in test_cases {
        ...
        let chapter_range_representation = BibleReferenceRepresentation::Range(BibleRange::ChapterRange(chapter_range.clone()));
        let expected_representation = BibleReferenceRepresentation::Range(BibleRange::ChapterRange(chapter_range));
...
fn test_upcast_single_verse_to_verse() {
    ...
    for (book, chapter, verse) in test_cases {
        ...
        let verse_range_representation = BibleReferenceRepresentation::Range(BibleRange::VerseRange(verse_range));
        let verse_representation = BibleReferenceRepresentation::Single(BibleReference::BibleVerse(verse_ref));
...
fn test_upcast_single_chapter_to_chapter() {
    ...
    for (book, chapter) in test_cases {
        ...
        let chapter_range = BibleChapterRange::new(chapter_ref.clone(), chapter_ref.clone()).unwrap();
        
        let chapter_range_representation = BibleReferenceRepresentation::Range(BibleRange::ChapterRange(chapter_range));
        let chapter_representation = BibleReferenceRepresentation::Single(BibleReference::BibleChapter(chapter_ref));
...
fn test_downcast_book_range_to_chapter_range() {
    ...
    for (start_book, end_book) in test_cases {
        ...
        let end_chapter = BibleChapterReference::new(end_book, get_number_of_chapters(&end_book)).unwrap();
...
fn test_as_list_for_book_range() {
    ...
    for (start_book, end_book) in test_cases {
        ...
        assert_eq!(
            book_list.first().unwrap(), &start_book_ref,
            "Book list doesn't start with the correct book"
        );
        assert_eq!(
            book_list.last().unwrap(), &end_book_ref,
...
fn test_as_list_for_chapter_range() {
    ...
    for (book, start_chapter, end_chapter) in test_cases {
        let start_chapter_ref = BibleChapterReference::new(book, start_chapter).unwrap();
        let end_chapter_ref = BibleChapterReference::new(book, end_chapter).unwrap();
        let chapter_range = BibleChapterRange::new(start_chapter_ref.clone(), end_chapter_ref.clone()).unwrap();
        ...
        assert_eq!(
            chapter_list.first().unwrap(), &start_chapter_ref,
            "Chapter list doesn't start with the correct chapter"
        );
        assert_eq!(
            chapter_list.last().unwrap(), &end_chapter_ref,
        ...
        assert_eq!(
            chapter_list.len(), expected_length,
...
fn test_as_list_for_verse_range() {
    ...
    for (book, chapter, start_verse, end_verse) in test_cases {
        ...
        let verse_range = BibleVerseRange::new(start_verse_ref.clone(), end_verse_ref.clone()).unwrap();
        ...
        assert_eq!(
            verse_list.first().unwrap(), &start_verse_ref,
            "Verse list doesn't start with the correct verse"
        );
        assert_eq!(
            verse_list.last().unwrap(), &end_verse_ref,
        ...
        assert_eq!(
            verse_list.len(), expected_length,
...
fn test_aggregate_adjacent_verses() {
    ...
    let expected = vec![BibleReferenceRepresentation::Range(BibleRange::VerseRange(expected_range))];
...
fn test_aggregate_adjacent_chapters() {
    ...
    let expected = vec![BibleReferenceRepresentation::Range(BibleRange::ChapterRange(expected_range))];
...
fn test_aggregate_adjacent_books() {
    ...
    let expected = vec![BibleReferenceRepresentation::Range(BibleRange::BookRange(expected_range))];
