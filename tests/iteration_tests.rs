...
fn test_iterate_all_books() {
    ...
    assert_eq!(all_books[0], BibleBook::Genesis, "The first book should be Genesis");
    
    // Check that the last book is Revelation
    assert_eq!(all_books[65], BibleBook::Revelation, "The last book should be Revelation");
...
fn test_iterate_old_testament_books() {
    ...
    let all_books = BibleBook::all();
    let old_testament_books: Vec<BibleBook> = all_books.into_iter()
        .filter(|book| book.is_old_testament())
        .collect();
    
    // Check that there are 39 books in the Old Testament
    assert_eq!(old_testament_books.len(), 39, "There should be 39 books in the Old Testament");
    
    // Check that the first book is Genesis
    assert_eq!(old_testament_books[0], BibleBook::Genesis, "The first book should be Genesis");
    
    // Check that the last book is Malachi
    assert_eq!(old_testament_books[38], BibleBook::Malachi, "The last book should be Malachi");
...
fn test_iterate_new_testament_books() {
    ...
    let all_books = BibleBook::all();
    let new_testament_books: Vec<BibleBook> = all_books.into_iter()
        .filter(|book| book.is_new_testament())
        .collect();
    
    // Check that there are 27 books in the New Testament
    assert_eq!(new_testament_books.len(), 27, "There should be 27 books in the New Testament");
    
    // Check that the first book is Matthew
    assert_eq!(new_testament_books[0], BibleBook::Matthew, "The first book should be Matthew");
    
    // Check that the last book is Revelation
    assert_eq!(new_testament_books[26], BibleBook::Revelation, "The last book should be Revelation");
...
fn test_iterate_book_range() {
    ...
    for (start_book, end_book, expected_count) in test_cases {
        ...
        assert_eq!(
            book_list.len(), expected_count,
            "Book range from {:?} to {:?} should have {} books",
            start_book, end_book, expected_count
        );
...
fn test_iterate_chapters_of_book() {
    ...
    for (book, expected_chapters) in test_cases {
        ...
        // Check that the list has the correct length
        assert_eq!(
            chapter_list.len(), expected_chapters as usize,
            "{:?} should have {} chapters",
            book, expected_chapters
        );
...
fn test_iterate_chapter_range() {
    ...
    for (book, start_chapter, end_chapter, expected_count) in test_cases {
        ...
        // Check that the list has the correct length
        assert_eq!(
            chapter_list.len(), expected_count,
            "Chapter range from {:?} {} to {} should have {} chapters",
            book, start_chapter, end_chapter, expected_count
        );
        ...
        for chapter_num in 1..=chapter_count {
            ...
            if chapter_num < chapter_count {
                ...
                assert_eq!(
                    ...
                    book, chapter_num, book, chapter_num + 1
                );
                ...
                let expected_next = BibleReference::BibleBook(
            ...
            if chapter_num > 1 {
                ...
                let expected_prev = BibleReference::BibleBook(
                    BibleBookReference::new(prev_book)
                );
                assert_eq!(
                    chapter_ref.previous(),
                    Some(expected_prev),
...
fn test_iterate_verses_of_chapter() {
    ...
    for (book, chapter, expected_verses) in test_cases {
        ...
        // Check that the list has the correct length
        assert_eq!(
            verse_list.len(), expected_verses.into(),
            "{:?} {} should have {} verses",
            book, chapter, expected_verses
        );
...
fn test_iterate_verse_range() {
    ...
    for (book, chapter, start_verse, end_verse, expected_count) in test_cases {
        ...
        // Check that the list has the correct length
        assert_eq!(
            verse_list.len(), expected_count,
            "Verse range from {:?} {}:{} to {}:{} should have {} verses",
            book, chapter, start_verse, chapter, end_verse, expected_count
        );
        ...
        for verse_num in 1..=verse_count {
            ...
            if verse_num < verse_count {
                ...
                assert_eq!(
                    ...
                    book, chapter, verse_num, book, chapter, verse_num + 1
                );
            } else {
                ...
                if next_chapter <= get_number_of_chapters(&book) {
                    ...
                    assert_eq!(
                        ...
                        book, chapter, book, next_chapter
                    );
                } else {
            ...
            if verse_num > 1 {
                ...
                if chapter > 1 {
                    ...
                    assert_eq!(
                        ...
                        book, chapter, book, prev_chapter
                    );
                } else {
